...
#define SGenFallbackForDefaultCC(CC) template<typename TRet, typename TO, typename ...TArgs>\
	struct S<TRet (CC TO::*) (TArgs...), typename std::enable_if<std::is_same<TRet (CC)(TArgs...), TRet(TArgs...)>::value>::type>\
	{\
...
#ifndef _M_X64
		SGenFallbackForDefaultCC(__cdecl); //Default fallbacks for defauld calling conventions
...
#define SMethodDetourTypeGen(CC) template<typename TRet, typename TObj, typename ...TArgs>\
	...
	{\
		using TDetour = std::function<TRet(TObj *pThis, TArgs...)>;\
	};
#ifndef _M_X64
...
#define SHookSpecMethodGen(CC) template<typename TRet, typename TObj, typename ...TArgs>\
	...
	{\
		...
		struct HelpTypes<TMethod, typename std::enable_if<std::is_member_function_pointer<TMethod>::value>::type>
		{
		...
		struct HelpTypes<TFunc, typename std::enable_if<!std::is_member_function_pointer<TFunc>::value>::type>
		{
			using TDetour = std::function<TFunc>;
			using THookSpec = typename S<TFunc>::template THookSpec<TFunc>;
		};

		template<typename T, typename = void> struct SCCStripper;
		template<typename T>
		struct SCCStripper<T, typename std::enable_if<!std::is_object<T>::value>::type>
		{
...
#define SGen(CC) template<typename TRet, typename TObj, typename ...TArgs>\
		...
		protected:
			...
			public:
				explicit SCounter(std::atomic_uint &i) : _i(i)
				{
					++_i;
				}
			...
			public:
				...
				explicit CMemoryFunction(std::initializer_list<unsigned __int8> &&byteCode, void *location = nullptr) :
...
#pragma pack(pop)
			...
			private:
				std::mutex m;
				std::list<COriginalBridge *> _availableBridges;
			public:
				COriginalBridge* get()
				{
		...
		private:
			...
			LPVOID detourProxyAddr()
			{
				...
				return std::is_same<TFunc, TRet __stdcall(TArgs ...)>::value ?
					(LPVOID*&)detourInvokerSTDCALL :
					std::is_same<TFunc, TRet __fastcall(TArgs ...)>::value ?
			...
			TFunc* originalFuncInvokerAddr()
			{
				...
				return std::is_same<TFunc, TRet __fastcall(TArgs ...)>::value ?
					(TFunc*&)originalInvokerFASTCALL :
					std::is_same<TFunc, TRet __cdecl(TArgs ...)>::value ?
			...
			CFunctionHookSpec(TFunc *target, std::function<TRet(TArgs...)> detour) : CFunctionHook(target), _detour(detour), _detourBridge(detourProxyAddr(), this)
			{
		...
		private:
			...
			LPVOID detourProxyAddr()
			{
				...
				return std::is_same<TMethod, TRet(__stdcall TObject::*)(TArgs ...)>::value ?
					(LPVOID*&)detourInvokerSTDCALL :
					std::is_same<TMethod, TRet(__fastcall TObject::*)(TArgs ...)>::value ?
					(LPVOID*&)detourInvokerFASTCALL :
					std::is_same<TMethod, TRet(__cdecl TObject::*)(TArgs ...)>::value ?
			...
			TOrigFunc* originalMethodInvokerAddr()
			{
				...
				return std::is_same<TOrigFunc, TRet __fastcall(TArgs ...)>::value ?
					(TOrigFunc*&)originalInvokerFASTCALL :
					std::is_same<TOrigFunc, TRet __cdecl(TArgs ...)>::value ?
		...
		std::unordered_map<LPVOID, CHook*> _hooks;
	...
	public:
		...
		{
			static_assert(std::is_same<typename SCCStripper<TDetour>::type, typename SCCStripper<typename HelpTypes<TMethod>::TDetour>::type>::value, "Unable to hook method: invalid detour signature! Must be TRet (TObject*, TArgs...)");
		...
		{
			static_assert(std::is_same<typename SCCStripper<TDetour>::type, typename SCCStripper<typename HelpTypes<TVirtualMethod>::TDetour>::type>::value, "Unable to hook virtual method: invalid detour signature! Must be TRet (TObject*, TArgs...)");
		...
		{
			static_assert(std::is_same<typename SCCStripper<TFunc>::type, typename SCCStripper<typename std::remove_pointer<TDetour>::type>::type>::value, "Unable to hook function: detour signature differs from target!");
